...
def multiGPU_CLIP(clip_model, images, text_tokens, target, device, prompt_token=None):
    ...
    if text_tokens.size()[0] == 1000:
        ...
        with torch.no_grad():
            for i in range(10):
                text_feature = clip_model.encode_text(text_tokens[100*i:100*(i+1),:])
        ...
        text_features = torch.tensor([]).to(device)
        with torch.no_grad():
            for i in range(math.ceil(text_tokens.size()[0] / 100)):
                if 100*(i+1) < text_tokens.size()[0]:
                    text_feature = clip_model.encode_text(text_tokens[100*i:100*(i+1),:])
                else:
                    text_feature = clip_model.encode_text(text_tokens[100*i:,:])
                text_features = torch.cat((text_features, text_feature), dim=0)
            text_features = text_features / text_features.norm(dim=-1, keepdim=True)
            scale_text_embed = clip_model.logit_scale.exp() * text_features
